%
% procSARdata_DBS_6272 v4 (De. 2012)
%
...
oversample_freq = 10;    % oversampling in Doppler; bigger makes better
% picture but needs more memory and time
fix_geometry = true;   % perform geometric corrections or not
...
% later. Dechirp the data first if that option selected. Note dechirp does
% use the correct range for each range bin.
...
if (fix_geometry)
    ...
    for n = 1:Nr
        ...
        % a little slower and not worth it just for display purposes
    end
    ...
    for m = 1:Nf
        ...
        ysarmag_rs2(:,m) = interp1(R,ysarmag_rs1(:,m),Ru,'linear',NaN);
        % cubic interpolation in the above line is somewhat better quality, but
        % a little slower and not worth it just for display purposes
    end
